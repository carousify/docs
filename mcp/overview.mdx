...
The Carousify MCP server is the developer-facing way to drive Carousify from any AI client that speaks the [Model Context Protocol](https://modelcontextprotocol.io/) — Claude, Cursor, ChatGPT, and other MCP-compatible clients.
...
The connection panel ships with built-in setup guides for **Claude**, **Cursor**, **VS Code (Cline)**, **ChatGPT**, and **Anti-Gravity**. Other MCP-compatible clients (Windsurf, Codex, custom agents) work too — the Carousify server uses an SSE transport, so any client that accepts an SSE-based MCP endpoint can connect by pasting the same URL.
...
- **Issue a new key** by typing a label into **API Name** (e.g. `claude-laptop`, `cursor-work`) and clicking **Create API Key**. Each key gets its own URL, so you can hand out one per client or per teammate and revoke them individually.
...
<Columns cols={2}>
  <Card title="Connect a client" icon="plug" href="/mcp/setup">
    Step-by-step setup for Claude, Cursor, Cline, ChatGPT, and Anti-Gravity.
  </Card>
